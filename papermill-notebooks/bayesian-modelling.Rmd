...
- [x] `limma::topTable()` dataframes written as csv, filenames shsould follow the convention: `se_{tissue_name}_AS_model_B_sex_as_events.csv`
...
```{r}
inc.counts.file.name  <- paste0(paste0(paste0(dataDir, "rmats_final."), splice_type),".jc.ijc.txt.gz")
inc.counts.file.name
file.exists(inc.counts.file.name)
```

```{r}
skip.counts.file.name  <- paste0(paste0(paste0(dataDir, "rmats_final."), splice_type),".jc.sjc.txt.gz")
