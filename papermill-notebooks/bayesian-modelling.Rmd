...
```{r}
as_site_types <- c("a3ss", "a5ss", "mxe", "ri", "se")
...
as_site_type <- as_site_types[splice_index] #can be replaced with a loop or argument to choose a different as_site_type
```
...
```{r}
inc.counts.file.name  <- paste0(paste0(paste0(dataDir, "rmats_final."), as_site_type),".jc.ijc.txt.gz")
inc.counts.file.name
file.exists(inc.counts.file.name)
```

```{r}
skip.counts.file.name  <- paste0(paste0(paste0(dataDir, "rmats_final."), as_site_type),".jc.sjc.txt.gz")
